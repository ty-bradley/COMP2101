...
if ! ssh remoteadmin@172.16.1.4 'sudo systemctl is-active ufw &> /dev/null'; then    # Run a sudo apt-get update for compatibility.
    echo -e "
${RED}UFW service not found on webhost; Installing.${RESET}"

    ssh remoteadmin@172.16.1.4 'sudo apt-get update &> /dev/null && sudo apt-get install -y ufw &> /dev/null'

    ssh remoteadmin@172.16.1.4 'sudo ufw allow 80/tcp &> /dev/null'

    # Start UFW service after installation
    ssh remoteadmin@172.16.1.4 'sudo systemctl start ufw &> /dev/null'
else
    # If UFW service is already running, display that message to the user.
    echo -e "
${BLUE}UFW service already installed on webhost; Restarting..${RESET}"
    ...
    ssh remoteadmin@172.16.1.4 'sudo ufw allow 80/tcp &> /dev/null'
    ssh remoteadmin@172.16.1.4 'sudo ufw allow 22/tcp &> /dev/null'
    ssh remoteadmin@172.16.1.4 'sudo systemctl restart ufw &> /dev/null'
...
if ! ssh remoteadmin@172.16.1.4 'systemctl is-active --quiet apache2'; then
    
    # Run a sudo apt-get update for compatibility.
    echo -e "
    ${RED}Apache2 web server service not found on webhost; Installing.${RESET}"
    
    ssh remoteadmin@172.16.1.4 "sudo apt-get update &> /dev/null && apt-get install -y apache2 &> /dev/null"

    # Start Apache2 service after installation
    ssh remoteadmin@172.16.1.4 'sudo systemctl start apache2 &> /dev/null'
else
    ...
    ssh remoteadmin@172.16.1.4 'sudo systemctl restart apache2 &> /dev/null'
fi

# Configure rsyslog on webhost to send logs to loghost
ssh remoteadmin@172.16.1.4 "sudo sed -i 's/^#module(load=\"imudp\")/module(load=\"imudp\")/' /etc/rsyslog.conf"
ssh remoteadmin@172.16.1.4 "sudo sed -i 's/^#input(type=\"imudp\" port=\"514\")/input(type=\"imudp\" port=\"514\")/' /etc/rsyslog.conf"
...
ssh remoteadmin@172.16.1.4 "echo '*.* @loghost' | sudo bash -c 'cat >> /etc/rsyslog.conf'"

#restart the rsyslog service using systemctl restart rsyslog

ssh remoteadmin@172.16.1.4 "sudo systemctl restart rsyslog"
...
ssh remoteadmin@172.16.1.4 "grep webhost /var/log/syslog"
